...
public:
    ...
    void setOnsets (const float* positions, int count)
    {
        if (positions != nullptr && count > 0)
            onsets_.assign (positions, positions + (size_t) count);
        else
            onsets_.clear();
    }
    ...
    void paint (juce::Graphics& g) override
    {
        ...
        g.setColour (juce::Colour (0xff5A8FA0));
        ...
        // ── Onset tick marks (orange) ─────────────────────────────────────────
        g.setColour (juce::Colour (0xffFF8800).withAlpha (0.75f));
        for (float pos : onsets_)
        {
            int ox = normToX (pos);
            g.drawVerticalLine (ox, 0.0f, 10.0f);
            g.drawVerticalLine (ox, (float)bh - 10.0f, (float)bh);
        }
        ...
        // ── Loop window shading (always visible) ─────────────────────────────
        {
            ...
            float winEnd;
            if ((isMoving || loopSizeLock_) && sampleRateHz_ > 0.0 && numSamplesTotal_ > 0)
        ...
        // ── Region handles (bright on dark) ───────────────────────────────────
        drawHandle (g, regionStart_, juce::Colour (0xffF2F2F7), bh);
        drawHandle (g, regionEnd_,   juce::Colour (0xffF2F2F7), bh);

        // ── Loop handles (blue, always visible) ───────────────────────────────
        drawHandle (g, loopStart_, juce::Colour (0xff4488EE), bh);
        if (!loopSizeLock_)
            drawHandle (g, loopEnd_, juce::Colour (0xff4488EE), bh);
        ...
        if (sampleRateHz_ > 0.0 && numSamplesTotal_ > 0)
        {
            ...
            if (loopMode_ != 0)
            {
                bool useMs = loopSizeLock_ || (loopMode_ == 2 || loopMode_ == 3);
                double lpMs = useMs
                    ? (double)loopSizeMs_
                    : (double)(loopEnd_ - loopStart_) * totalMs;
...
private:
    ...
    void drawHandle (juce::Graphics& g, float norm, juce::Colour col, int h)
    {
        ...
        // Larger top triangle (14px wide, 18px tall) — easier to grab
        juce::Path tri;
        tri.addTriangle ((float)(x - 7), 0.0f,
                         (float)(x + 7), 0.0f,
                         (float) x,       18.0f);
        g.fillPath (tri);
        // Bottom triangle pointing up for end handles
        juce::Path btri;
        btri.addTriangle ((float)(x - 7), (float) h,
                          (float)(x + 7), (float) h,
                          (float) x,       (float)(h - 18));
        g.fillPath (btri);
    }
